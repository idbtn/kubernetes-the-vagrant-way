...
# Join new slave node to existing cluster.
# TODO Get apiserver advertise address from somewhere.
sudo kubeadm join loadbalancer1.idbtn.net:6443 --token $(cat ${token_path}) --discovery-token-ca-cert-hash sha256:$(cat ${ca_cert_hash_path})

echo "========================="
echo "Node $(hostname -f) joined the cluster."
echo "========================="
